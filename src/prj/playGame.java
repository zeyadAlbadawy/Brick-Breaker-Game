...
public class playGame extends JPanel implements KeyListener, ActionListener {
	...
	// To not make the game start by itself
	...
	private Timer timer;
	private int delay = 4; // The speed of the ball
	...
	public void actionPerformed(ActionEvent e) {
		...
		repaint();
		if (play) {
			// For Detecting Hitting The paddle
			...
			A: for (int i = 0; i < map.map.length; i++) {
				for (int j = 0; j < map.map[0].length; j++) {
					if (map.map[i][j] > 0) {
						// 80, 50 for the black space before the breaks (left, top )
						...
						// rect object refer to the whole rectangle.

						Rectangle rect = new Rectangle(brickX, brickY, brickWidth, brickHeight);
						Rectangle ballRect = new Rectangle(ballposX, ballposY, 20, 20);
						Rectangle brickRect = rect;

						// when the ball hits the maps
						if (ballRect.intersects(brickRect)) {
							
							// passes the 0 to map generator to not be able to create white blocks and
							// create black space.
							...
							// to not break all rectangles by one collosion
	...
	// keyTyped - when the unicode character represented by this key is sent by the
	...
	public void paint(Graphics g) {
		...
		// create the borders ( left >> Right >> Top)
		...
		// 530 from the Top and 100 For Height and 8 for width
		g.fillRect(playerX, 547, 100, 8);
		...
		if (totalBricks <= 0) {
			play = false;
			// To Hide The ball when win
			ballXdir = 0;
			ballYdir = 0;
			g.setColor(Color.decode("#ADA2FF"));
